...
struct mpp : predictor {
    ...
    static constexpr u64 LINE_WEIGHT = LINEINST >> 1;    // 2 Offsets share a weight
    ...
    arr<reg<WBITS, i64>, NTABLES> readw[LINE_WEIGHT];    // 2 Offsets share a weight
    ...
    // ---- RAMs ----
    ram<val<WBITS, i64>, (1 << index2_bits)> wtable[NTABLES][LINE_WEIGHT] {{"P2 weight"}};
    ...
    void update_cycle(instruction_info &block_end_info)
    {
        ...
        for (u64 offset=0; offset < LINE_WEIGHT; offset++) {
            ...
            execute_if(train[2*offset]|train[2*offset+1], [&](){
                for (u64 i=0; i<NTABLES; i++) {
                    wtable[i][offset].write(index2[i], update_ctr(readw[offset][i], ~(branch_taken[2*offset]|branch_taken[2*offset+1])));
                }
            });
        }
